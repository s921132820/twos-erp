    ...
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 bg-slate-900 text-white lg:block">
      ...
      <nav aria-label="주 메뉴" className="space-y-1 p-3">
        {menus.map(({ href, label, icon: Icon, active }) => (
          <Link key={href} href={href} prefetch={false} className={cn("flex items-center gap-3 rounded-md px-4 py-3 text-sm font-semibold text-slate-300 transition-colors hover:bg-slate-800 hover:text-white", active && "bg-blue-600 text-white shadow-sm hover:bg-blue-600")} aria-current={active ? "page" : undefined}>
