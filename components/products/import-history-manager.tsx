...
function HistoryForm({ productId, history, onSuccess, onCancel }: { productId: string; history?: ImportLivestockHistory; onSuccess: (message: string) => void; onCancel: () => void }) {
  ...
  return <form action={(formData) => startTransition(() => formAction(formData))} className="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <label className="text-sm font-medium text-slate-700">수입일자 <span className="text-red-500">*</span><Input type="date" name="importDate" value={values.importDate} onChange={(event) => change("importDate", event.target.value)} className="mt-1.5 bg-white" /><FieldError messages={state.errors?.importDate} /></label>
      <label className="text-sm font-medium text-slate-700">원산지 <span className="text-red-500">*</span><Input name="countryOfOrigin" value={values.countryOfOrigin} onChange={(event) => change("countryOfOrigin", event.target.value)} maxLength={100} className="mt-1.5 bg-white" /><FieldError messages={state.errors?.countryOfOrigin} /></label>
      <label className="text-sm font-medium text-slate-700">공급처 <span className="text-red-500">*</span><Input name="supplierName" value={values.supplierName} onChange={(event) => change("supplierName", event.target.value)} maxLength={150} className="mt-1.5 bg-white" /><FieldError messages={state.errors?.supplierName} /></label>
      ...
      <label className="text-sm font-medium text-slate-700">B/L번호<Input name="billOfLadingNumber" value={values.billOfLadingNumber} onChange={(event) => change("billOfLadingNumber", event.target.value)} maxLength={100} className="mt-1.5 bg-white" /><FieldError messages={state.errors?.billOfLadingNumber} /></label>
...
export function ImportHistoryManager({ product }: { product: ProductWithImportHistories }) {
  ...
  return <div className="space-y-5">
    ...
    {showForm && <HistoryForm key={editing?.id ?? "new"} productId={product.id} history={editing} onSuccess={complete} onCancel={() => { setShowForm(false); setEditing(undefined); }} />}
    {product.importLivestockHistories.length === 0 ? <div className="flex min-h-56 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white text-sm text-slate-500">등록된 수입축산물 이력이 없습니다.</div> : <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white"><table className="w-full min-w-[1200px] text-left text-sm"><thead className="bg-slate-50 text-xs font-semibold text-slate-500"><tr>{["이력번호", "수입일자", "원산지", "공급처", "수입 상세정보", "메모", "상태", "관리"].map((item) => <th key={item} className="px-5 py-3.5">{item}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{product.importLivestockHistories.map((history) => <tr key={history.id}><td className="px-5 py-4 font-semibold text-slate-900">{history.historyNumber}</td><td className="px-5 py-4 text-slate-600">{new Intl.DateTimeFormat("ko-KR").format(history.importDate)}</td><td className="px-5 py-4 text-slate-600">{history.countryOfOrigin}</td><td className="px-5 py-4 text-slate-600">{history.supplierName}</td><td className="min-w-72 px-5 py-4 text-xs leading-5 text-slate-600"><p><strong>품목:</strong> {history.itemName || "-"}</p><p><strong>B/L:</strong> {history.billOfLadingNumber || "-"}</p><p><strong>수출업체:</strong> {history.exporterName || "-"}</p><p><strong>도축장:</strong> {history.foreignSlaughterhouse || "-"}</p><p><strong>가공장:</strong> {history.foreignProcessingPlant || "-"}</p><p><strong>부위:</strong> {history.partNameCode || "-"}</p><p><strong>도축일자:</strong> {history.foreignSlaughterDate ? history.foreignSlaughterDate.toISOString().slice(0, 10) : "-"}</p></td><td className="max-w-60 whitespace-pre-wrap px-5 py-4 text-slate-600">{history.memo || "-"}</td><td className="px-5 py-4"><span className={history.isActive ? "rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700" : "rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-500"}>{history.isActive ? "사용" : "미사용"}</span></td><td className="whitespace-nowrap px-5 py-4"><Button size="sm" variant="ghost" onClick={() => { setEditing(history); setShowForm(true); }}>수정</Button><Button size="sm" variant="ghost" className="text-red-600" disabled={deleting} onClick={() => remove(history)}>삭제</Button></td></tr>)}</tbody></table></div>}
  </div>;
}
