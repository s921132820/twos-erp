...
export function ShippingLabelConverter() {
  ...
  const resetAll = () => { requestIds.current.meatbox += 1; requestIds.current["coupang-wing"] += 1; requestIds.current["smart-store"] += 1; setMeatboxState(createInitialUploadState()); setCoupangWingState(createInitialUploadState()); setSmartStoreState(createInitialUploadState()); setManualRows([]); setOverrides({}); setExcludedRowKeys(new Set()); setEditingRow(null); closeManualDialog(); };
